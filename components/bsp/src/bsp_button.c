...
// 只保留一次物理按键对应的【一次】回调:
//   - BUTTON_SINGLE_CLICK → 单击(菜单上下移动/进入, demo 内触发动作)
//   - BUTTON_LONG_PRESS_START → 长按(demo 内返回菜单)
// 注意: 切勿再注册 BUTTON_PRESS_DOWN —— 否则一次按压会同时触发 PRESS+CLICK 两次,
//       让菜单单次 UP/DOWN 跨越两行。iot_button 对长按仍会先触发 LONG,不依赖 PRESS。
